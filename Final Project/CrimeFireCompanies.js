...
let state = {
   ...
   Promise.all([
    d3.json("../data/ProjectDataFinal/CrimeFireCompanies.geojson"),
    d3.csv("../data/ProjectDataFinal/csv/HateCrimeFireCompanies.csv")
...
function init() {
    ...
    .attr("fill", (d, i) => {
      ...
      //console.log(noiseLookup.get(d.properties.fire_co_nu))
      return colorScale(+noiseLookup.get(d.properties.fire_co_nu.toString()))
    })     
    .attr("stroke", "grey")
    ...
    statenIslandBoundry.on("mousemove", (ev, d) => {
     ...
     state.hover_state = parseInt(d.properties.fire_co_nu);
...
function draw() {
  ...
  .style("left", 20 + "px")
    .html(
      `<div>Average Number of Hate Crimes Per Fire Company District:11 </div>
      <div>Fire Company: ${state.hover_state}</div>
      <div>Hate Crime: ${state.hover_population}</div>`
    )
  }
