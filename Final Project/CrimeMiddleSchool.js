...
let state = {
   ...
   Promise.all([
    d3.json("../data/ProjectDataFinal/CrimeMiddleSchool.geojson"),
    d3.csv("../data/ProjectDataFinal/csv/HateCrimeMiddleSchool.csv")
...
function init() {
    ...
    .attr("fill", (d, i) => {
      ...
      return colorScale(+noiseLookup.get(d.properties.dbn))
    })     
    .attr("stroke", "grey")
...
function draw() {
  ...
  .style("left", 20 + "px")
    .html(
      `<div>Average Number of Hate Crimes Per Middle School District: 29 </div>
      <div>Middle School: ${state.hover_state}</div>
