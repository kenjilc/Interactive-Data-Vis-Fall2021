...
let state = {
   ...
   Promise.all([
    ...
    d3.json("../data/ProjectDataFinal/CrimeElectionDistrict.geojson"),
    d3.csv("../data/ProjectDataFinal/csv/HateCrimeElectionDistrict.csv")
...
function init() {
    ...
    const noiseLookup = new Map(state.hateCrimeMapsCSV.map(d => [
        parseInt(d['elect_dist']).toString(), d['AboveAverage']
      ]))
    ...
    .attr("fill", (d, i) => {
      ...
      //console.log(noiseLookup.get(d.properties.elect_dist.toString()))
      return colorScale(+noiseLookup.get(parseInt(d.properties.elect_dist).toString()))
    })
    .attr("stroke", "grey")  
    ...
    statenIslandBoundry.on("mousemove", (ev, d) => {
     ...
     state.hover_state = parseInt(d.properties.elect_dist);
...
function draw() {
  ...
  .style("left", 15+ "px")
    .html(
      `<div>Average Number of Hate Crimes Per Election District: 1</div
      <div>Election District: ${state.hover_state}</div>
