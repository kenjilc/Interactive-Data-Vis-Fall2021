...
let state = {
   ...
   Promise.all([
    d3.json("../data/ProjectDataFinal/NoiseHealthAreas.geojson"),
    d3.csv("../data/ProjectDataFinal/csv/NoiseHealthAreas.csv"),
...
function init() {
    ...
    .attr("fill", (d, i) => {
      ...
      //console.log(noiseLookup.get(d.properties.health_are))
      return colorScale(+noiseLookup.get(d.properties.health_are.toString()))
    })     
    .attr("stroke", "grey")
      // Hate Crime Points
    ...
    //Retrieve Geographical Boundary Data
    statenIslandBoundry.on("mousemove", (ev, d) => {
      //console.log('d :>> ', d);
      //console.log(d.properties.precinct)
      //console.log(d.properties.Count_)
     state.hover_state = parseInt(d.properties.health_are);
...
function draw() {
  ...
  .style("left", 20 + "px")
    .html(
    ` <div>Average Number of Noise Complaints Per Health Area: 120901</div>
      <div>Health Area: ${state.hover_state}</div>
