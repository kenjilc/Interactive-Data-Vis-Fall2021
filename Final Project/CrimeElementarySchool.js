...
let state = {
   ...
   Promise.all([
    d3.json("../data/ProjectDataFinal/CrimeElementarySchool.geojson"),
    d3.csv("../data/ProjectDataFinal/csv/HateCrimeElementarySchool.csv")
...
function init() {
    ...
    statenIslandBoundry.on("mousemove", (ev, d) => {
     ...
     state.hover_state =  parseInt(d.properties.label);
...
function draw() {
  ...
  .style("left", 20 + "px")
    .html(
      `<div>Average Number of Hate Crimes Per Elementary School District: 7 </div>
      <div>Elementary School Districts: ${state.hover_state}</div>
